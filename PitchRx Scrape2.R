scrape2 <- function (start, end, game.ids, suffix = "inning/inning_all.xml", 
          connect, ...) 
{
  if (!missing(connect)) {
  ...
  if (missing(game.ids)) {
    gameDir <- pitchRx::makeUrls(start = start, end = end)
  }
  else {
    ...
    gameDir <- pitchRx::makeUrls(gids = game.ids)
  }
  fields = NULL
  env2 <- environment()
  ...
  if (any(grepl("miniscoreboard.xml", suffix))) {
    ...
    scoreboards <- paste0(dayDir, "/epg.xml")
    obs <- XML2R::XML2Obs(scoreboards, as.equiv = TRUE, url.map = FALSE, 
                   ...)
    illegal <- paste0("epg//game//", c("review", "home_probable_pitcher", 
                                         "away_probable_pitcher"))
    obs <- obs[!names(obs) %in% illegal]
    nms <- names(obs)
    nms <- gsub("^epg//game//game_media//media$", "media", 
                nms)
    nms <- gsub("^epg//game$", "game", nms)
  ...
  if (any(grepl("inning/inning_all.xml", suffix))) {
    ...
    for (i in seq_len(n.loops)) {
      inning.filez <- inning.files[seq(1, cap) + (i - 
                                                    1) * cap]
      ...
      tab.nms <- sub("^game//inning//atbat$", "atbat", 
                     tab.nms)
      tab.nms <- sub("^game//inning//atbat//action$", 
                     "action", tab.nms)
      tab.nms <- sub("^game//inning//atbat//po$", "po", 
                     tab.nms)
      tab.nms <- sub("^game//inning//atbat//runner$", 
                     "runner", tab.nms)
      ...
      colnames(tables[["atbat"]]) <- sub("^pitcher$", 
                                         "id", colnames(tables[["atbat"]]))
      tables[["atbat"]] <- merged(x = tables[["atbat"]], 
                                  y = players, by = "id", all.x = TRUE)
